...
export const StudentDashboard = () => {
  ...
  const hasSecuredHousing = !!residency?.property
    ...
    <div className="flex bg-[#F4F8F5] min-h-screen font-dm-sans overflow-x-hidden">
      ...
      <main className="flex-1 md:ml-64 p-4 sm:p-6 md:p-8 min-h-screen relative z-10 pb-safe md:pb-8 pt-32 md:pt-32">
        {isSyncing ? (
          ...
          <>
            <PageHeader 
              title={hasSecuredHousing ? `Welcome Home, ${(user?.name || 'Resident').split(' ')[0]}` : "Your Hub"}
              subtitle="Muzinda Concierge • Premium Housing Management"
            />
          ...
          {hasSecuredHousing ? (
            ...
            <>
              ...
              <motion.section variants={itemVariants} className="lg:col-span-8 space-y-6 md:space-y-8">
                ...
                {hasSecuredHousing && residency?.property && (
                  <div className="relative group overflow-hidden rounded-[2.5rem] md:rounded-[3rem] border border-primary/10 shadow-2xl bg-white min-h-[350px] flex flex-col md:flex-row mb-12">
                     {/* Property Image Side */}
